...
import os
load_dotenv("klc0306.env",override=True)
...
if __name__ == "__main__":
    ...
    if True:
        ...
        save_instances(instances, file_name='data_sBB_005.pkl')

    # solve mnl
    if True:
        solve_mnl(input_file='data_sBB_005.pkl', output_file='data_sBB_005.pkl')
    
    # Solve the exact sp via spatial branch-and-bound
    if True:
        solve_exact_sp(input_file='data_sBB_005.pkl', output_file='data_sBB_005.pkl', tolerance=0.05)

    # Solve the exact rsp via spatial branch-and-bound
    if True:
        solve_exact_rsp(input_file='data_sBB_005.pkl', output_file='data_sBB_005.pkl', tolerance=0.05)

    # Solve the clustered sp via spatial branch-and-bound
    if True:
        solve_clustered_sp(input_file='data_sBB_005.pkl', output_file='data_sBB_005.pkl', tolerance=0.01)

    # Solve the clustered rsp via spatial branch-and-bound
    if True:
        solve_clustered_rsp(input_file='data_sBB_005.pkl', output_file='data_sBB_005.pkl', tolerance=0.01)
    
    # Solve with brute force and calculate gaps
    if True:
        solve_brute_force_and_greedy(
            input_file='data_sBB_005.pkl',
            output_file='data_sBB_005.pkl',
