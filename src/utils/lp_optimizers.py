...
class LinearProgramSolver:
    ...
    def maximize(self, 
        ...
        """Solve a linear maximization problem with variables in [0,1]
    ...
    def _solve_pulp(self, c: np.ndarray, A: Optional[np.ndarray], b: Optional[np.ndarray]) -> Tuple[float, np.ndarray, str]:
        """Solve using PuLP"""
        N = len(c)
        model = LpProblem(name="linear_program", sense=LpMaximize)
        # 主要变化：使用连续变量，范围在[0,1]之间
        x = [LpVariable(name=f'x_{i}', lowBound=0, upBound=1) for i in range(N)]
