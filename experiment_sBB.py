...
if __name__ == "__main__":
    ...
    if True:
        ...
        save_instances(instances, file_name='data_sBB.pkl')

    # solve mnl
    if True:
        solve_mnl(input_file='data_sBB.pkl', output_file='data_sBB.pkl')
    
    # Solve the exact sp via spatial branch-and-bound
    if True:
        solve_exact_sp(input_file='data_sBB.pkl', output_file='data_sBB.pkl', tolerance=0.03)

    # Solve the exact rsp via spatial branch-and-bound
    if True:
        solve_exact_rsp(input_file='data_sBB.pkl', output_file='data_sBB.pkl', tolerance=0.03)

    # Solve the clustered sp via spatial branch-and-bound
    if True:
        solve_clustered_sp(input_file='data_sBB.pkl', output_file='data_sBB.pkl', tolerance=0.01)

    # Solve the clustered rsp via spatial branch-and-bound
    if True:
        solve_clustered_rsp(input_file='data_sBB.pkl', output_file='data_sBB.pkl', tolerance=0.01)
    
    # Solve with brute force and calculate gaps
    if True:
        solve_brute_force_and_greedy(
            input_file='data_sBB.pkl',
            output_file='data_sBB.pkl',
